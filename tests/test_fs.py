...
def test_real_file_system_should_return_empty_directory(filesystem):
    result = filesystem.get_dir_list(DirListingRequest(Path(".")))
    assert not result


def test_real_file_system_should_block_absolute_urls(filesystem):
    with pytest.raises(ValueError):
        filesystem.get_dir_list(DirListingRequest(Path("/test")))


def test_real_file_system_should_block_urls_outside_root(filesystem):
    with pytest.raises(ValueError):
        filesystem.get_dir_list(DirListingRequest(Path("../../test")))


def test_real_file_system_should_return_files(temporary_dir, filesystem):
